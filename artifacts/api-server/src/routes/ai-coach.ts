...
async function callGeminiModel(model: string, prompt: string) {
  ...
  if (!response.ok) {
    throw new Error(`Gemini ${model} HTTP ${response.status}: ${String(data?.error?.message || data?.error?.status || "Gemini API error")}`);
  }
...
async function callGemini(prompt: string) {
  try {
    ...
    if (
      ...
      (message.includes("HTTP 404") || message.includes("HTTP 429") || message.toLowerCase().includes("quota") || message.toLowerCase().includes("rate limit"))
    ) {
      console.warn("Gemini primary model unavailable; trying Flash-Lite:", message);
...
async function handleCoach(req: Request, res: Response, requireAuth = false) {
  try {
    ...
    if (message.includes("429") || message.toLowerCase().includes("rate limit") || message.toLowerCase().includes("quota")) {
      return res.status(429).json({ error: message, diagnostic: true });
    }
