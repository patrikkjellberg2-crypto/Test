...
import { getThMatchupStats } from "../lib/war-archive";
...
- Keep reasons short.
- If HISTORICAL PERFORMANCE data is provided, use it: favor attackers against Town Hall levels where the clan historically gets a high average or a high 3-star rate, and be more cautious recommending a clean 3-star attempt against levels where the average has been low.
- Respond in Swedish.
...
router.post("/ai/war-planner", async (req: Request, res: Response) => {
  try {
    ...
    // Best-effort: add historical TH matchup performance from this clan's
    // own war archive. Never let this fail the planner if it errors out.
    let historyBlock = "";
    try {
      const history = await getThMatchupStats(clanTag);
      if (history.length) {
        historyBlock = `\n\nHISTORICAL PERFORMANCE (this clan, from stored past wars):\n${JSON.stringify(
          history,
        )}`;
      }
    } catch (error) {
      console.warn("War planner: could not load historical TH matchup stats", error);
    }

    const output = await callOpenAI(
      SYSTEM_PROMPT,
      `CURRENT WAR DATA:\n${warData}${historyBlock}`,
    );
