...
function orientWarToClan(
  ...
  const clanIsRequested =
    normalizeClanTag(String(clan.tag ?? "")) === requested;
  const opponentIsRequested =
    normalizeClanTag(String(opponent.tag ?? "")) === requested;

  if (!clanIsRequested && !opponentIsRequested) return null;

  const ourSide = clanIsRequested ? clan : opponent;
  const otherSide = clanIsRequested ? opponent : clan;

  return {
    ...war,
    // Always archive from our clan's perspective. ClashKing may return the
    // two sides in either order; without this normalization the previous war
    // can be archived under the opponent tag and Player History returns 0.
    clan: ourSide,
    opponent: otherSide,
    result: normalizeWarState(war, clanTag),
  };
}
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    // The previous-war endpoint is a full ClanWar object with member-level
    // attack detail. Backfill it into the archive so Player Cards still have
    // history when the app was not open during the war's final state.
    const previousWarRaw = normalizeWarLog(
      listItems(previousWarResult.data),
      clanTag,
    )[0] ?? null;
    const previousWar = previousWarRaw
      ? orientWarToClan(previousWarRaw, clanTag)
      : null;

    if (previousWar) {
      await snapshotHistoricalWar(clanTag, previousWar, req.log);
    }
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      let archivedHistory = await getPlayerWarHistory(
        clanTag,
        tag,
        20,
      );

      // A player page can be opened directly, without a dashboard request
      // having warmed the archive first. Repair the latest completed war here
      // as well, using ClashKing's full previous-war response.
      if (archivedHistory.wars === 0) {
        const previousWarResult = await fetchOptionalClashKingResource(
          `/war/${encodeURIComponent(clanTag)}/previous`,
          [],
          req.log,
        );
        const previousWarRaw = normalizeWarLog(
        ...
        if (previousWar) {
          await snapshotHistoricalWar(clanTag, previousWar, req.log);
          archivedHistory = await getPlayerWarHistory(
            clanTag,
            tag,
            20,
          );
      ...
      // Prefer server-side archived wars because they contain full member and
      // attack detail. Fall back to the official war log only for installations
      // that have not captured any historical war yet.
      const warlog =
        archivedHistory.wars > 0
          ? null
      ...
      const history = warlog
        ? listItems(warlog.data)
        : [];
      ...
      if (archivedHistory.wars > 0) {
        res.json({
          ...player,
          historicalWarStats: archivedHistory,
        });
        return;
      }
