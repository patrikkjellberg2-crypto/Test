    ...
    return (
      <article className="premium-card rounded-2xl border border-sky-400/15 p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold">Intelligence Pulse</h2>
            <p className="text-xs text-muted-foreground">No active war board available.</p>
          </div>
        </div>
  ...
  return (
    <article className="premium-card overflow-hidden rounded-2xl border border-sky-400/15">
      <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
        ...
        <Link
          href="/war-planner"
          className="inline-flex items-center gap-1.5 rounded-lg bg-primary/15 px-3 py-1.5 text-[10px] font-bold text-sky-200"
        >
      ...
      <div className="grid gap-3 p-5 md:grid-cols-3">
        <div className="rounded-xl border border-white/10 bg-white/[.025] p-4">
          <p className="text-[10px] font-bold uppercase tracking-[.15em] text-muted-foreground">Board</p>
          <p className="mt-2 text-2xl font-bold">{n(own.stars)}–{n(opponent.stars)}</p>
          <p className="mt-1 text-xs text-muted-foreground">
            {active ? `${ownRemaining} own · ${enemyRemaining} enemy attacks left` : "Latest captured state"}
          </p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/[.025] p-4">
          <p className="text-[10px] font-bold uppercase tracking-[.15em] text-muted-foreground">Destruction</p>
          <p className="mt-2 text-2xl font-bold">{n(own.destructionPercentage).toFixed(1)}%</p>
          <p className="mt-1 text-xs text-muted-foreground">
            Opponent {n(opponent.destructionPercentage).toFixed(1)}%
          </p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/[.025] p-4">
          <p className="text-[10px] font-bold uppercase tracking-[.15em] text-muted-foreground">Detected</p>
          <p className="mt-2 text-2xl font-bold">{alerts.length}</p>
          <p className="mt-1 text-xs text-muted-foreground">data-backed signals</p>
        </div>
      </div>
